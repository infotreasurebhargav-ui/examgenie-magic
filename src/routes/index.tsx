...
import {
  Sparkles, Download, Link2, FileText, Eye, Pencil,
...
// ─── Sidebar ──────────────────────────────────────────────────────────────────
const NAV_ITEMS = [
  { icon: LayoutDashboard, label: "Generate Paper", active: true },
  { icon: BookOpen,        label: "Question Bank",  active: false, soon: true },
  { icon: Users,           label: "Students",        active: false, soon: true },
    ...
    <>
      ...
      `}>
        ...
        <div className="flex items-center gap-3 border-b border-white/10 px-5 py-5">
          ...
          <button onClick={onClose} className="ml-auto rounded-lg p-1.5 text-slate-500 hover:bg-white/5 hover:text-white lg:hidden">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="mx-4 mt-4 rounded-xl border border-indigo-500/20 bg-indigo-500/10 px-3 py-2.5">
          ...
          <div className="mt-0.5 truncate text-sm font-semibold text-white">St. Xavier's High School</div>
        </div>
            ...
            <button
              ...
              className={`
                ...
                ${item.active
                  ...
                  : "text-slate-500 hover:bg-white/5 hover:text-slate-300 disabled:opacity-40 disabled:cursor-not-allowed"}
              `}
            >
              <item.icon className="h-4 w-4 shrink-0" />
              <span className="flex-1 text-left">{item.label}</span>
              {item.soon && (
                <span className="rounded-full bg-slate-800 px-1.5 py-0.5 text-[9px] uppercase tracking-wider text-slate-500 font-semibold">
                  Soon
                </span>
              )}
              {item.active && <ChevronRight className="h-3.5 w-3.5 opacity-60" />}
            </button>
          ))}
        </nav>
...
function Stepper({ step, hasPaper }: { step: Step; hasPaper: boolean }) {
  ...
  const done = hasPaper ? (step === "review" ? 1 : 0) : 0;

  return (
    <div className="hidden items-center gap-1 sm:flex">
      {steps.map((s, i) => {
        const isDone = i < done || (hasPaper && i <= 1 && step === "review" && i < 1) || (hasPaper && step === "review" && i === 2);
          ...
          {step === "review" && paper && (
            <div className="flex flex-col gap-4">
              ...
              <div className="grid gap-4 lg:grid-cols-[400px_1fr] xl:grid-cols-[440px_1fr]">
                ...
                {/* ── Left: Editor ── */}
                <div className="flex flex-col gap-0 overflow-hidden rounded-2xl border border-white/10 bg-white/3">
                ...
                <div className="flex flex-col gap-0 overflow-hidden rounded-2xl border border-white/10 bg-white/3">
                  <div className="flex items-center gap-2 border-b border-white/10 bg-white/5 px-4 py-3">
                    ...
                    <div className="ml-auto">
                      <Tabs
                        value={showAnswers ? "key" : "paper"}
                        onValueChange={(v) => setShowAnswers(v === "key")}
                      >
              ...
              <div className="rounded-2xl border border-white/10 bg-white/3 px-5 py-4">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div>
                    ...
                    <div className="text-xs text-slate-500">Send students a live online test, or download a print-ready PDF.</div>
                  </div>
                  <div className="flex flex-wrap gap-3">
                    ...
                    >
                      <Link2 className="h-4 w-4" />
                      Share Online Test
                      <span className="rounded-full bg-indigo-500/20 px-1.5 py-0.5 text-[9px] font-semibold text-indigo-300 uppercase tracking-wider">Live</span>
                    </Button>
                    <Button
                      onClick={handleDownload}
